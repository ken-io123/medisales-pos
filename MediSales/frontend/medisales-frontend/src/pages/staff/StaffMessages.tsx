...
import { Search, Send, MessageCircle, Check, X, MoreVertical, Edit2, Trash2, Phone, Video, Info } from 'lucide-react';
...
const StaffMessages = () => {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] bg-white overflow-hidden rounded-2xl border-2 border-slate-200 shadow-lg m-4">
      {/* Sidebar (Hardcoded for Staff -> Admin) */}
      <div className="w-80 bg-white border-r-2 border-slate-100 flex flex-col hidden md:flex">
        {/* Sidebar Header */}
        <div className="p-6 border-b-2 border-slate-100 bg-slate-50/50">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Support Chat</p>
              </div>
            </div>
          </div>
          ...
          <div className="relative group">
            ...
            <input
              ...
              placeholder="SEARCH..."
              disabled
              className="w-full pl-10 pr-4 py-2.5 bg-white border-2 border-slate-200 rounded-xl text-xs font-bold uppercase tracking-wide text-slate-700 placeholder:text-slate-400 focus:outline-none cursor-not-allowed opacity-70"
            />
          </div>
        </div>

        {/* Conversations List (Single Item) */}
        <div className="flex-1 overflow-y-auto p-2">
          <button
            className="w-full p-4 flex items-start gap-3 transition-all bg-blue-50/50 rounded-xl border-2 border-blue-100 group relative"
          >
            ...
            <div className="relative">
              ...
              />
              <span className={`absolute -bottom-1 -right-1 w-4 h-4 border-2 border-white rounded-full flex items-center justify-center ${isConnected ? 'bg-emerald-500' : 'bg-slate-300'}`}>
                {isConnected && <span className="w-1.5 h-1.5 bg-white rounded-full animate-pulse" />}
              </span>
            </div>
            <div className="flex-1 min-w-0 text-left pt-1">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider bg-white px-1.5 py-0.5 rounded border border-slate-100">
                  NOW
                </span>
              </div>
              <div className="flex items-center justify-between">
                <p className="text-xs font-bold text-slate-700 truncate">
                  {messages.length > 0 ? messages[messages.length - 1].messageText : 'Start a conversation...'}
                </p>
              </div>
            </div>
          </button>
