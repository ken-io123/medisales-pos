...
type EditProductModalProps = {
  ...
  categories: string[];
  onClose: () => void;
  onSave: (productId: number, payload: ProductPayload) => Promise<void> | void;
};
...
const EditProductModal = ({ open, product, categories, onClose, onSave }: EditProductModalProps) => {
  ...
  const normalizedCategories = useMemo(() => Array.from(new Set([...(categories || []), product?.category ?? ''])).filter(Boolean), [categories, product]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (caughtError instanceof Error) {
        setError(caughtError.message || 'Unable to update product.');
      } else {
      ...
      <div className="w-full max-w-3xl rounded-3xl bg-white shadow-2xl">
        <div className="flex items-center justify-between p-8 pb-4">
          <div>
            ...
            <p className="text-sm text-brand-muted">Update product information to keep the inventory accurate.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex justify-end gap-3 px-8 pb-8 pt-4">
          ...
          <button
            ...
            className="rounded-2xl bg-blue-600 px-6 py-2.5 text-sm font-semibold text-white shadow-lg shadow-blue-500/30 transition hover:bg-blue-700 disabled:opacity-75"
            disabled={saving}
          >
